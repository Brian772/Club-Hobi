...
@section('title', 'Orbii | Audit Log Details')

@section('content')
  <header class="flex flex-row gap-2 lg:gap-4 items-center justify-start mb-6">
    <a href="{{ route('admin.audit-logs') }}" class="text-ink-muted">
    ...
    <h2 class="text-title lg:text-2xl font-semibold flex flex-row items-center gap-2 justify-center text-ink">Audit Log
      Details</h2>
    </h2>
  </header>
  ...
  <main class="flex flex-col gap-4">
    <section class="w-full p-6 rounded-lg border border-hairline">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="flex flex-col gap-1">
          ...
          <span class="text-caption text-ink">{{ $auditLog->user?->name ?? 'System' }}</span>
        </div>
        <div class="flex flex-col gap-1">
          <p class="text-ink-muted font-semibold text-body-mid">Action:</p>
          <span class="text-caption text-ink">{{ $auditLog->action }}</span>
        </div>
        <div class="flex flex-col gap-1">
          <p class="text-ink-muted font-semibold text-body-mid">Target:</p>
          <span class="text-caption text-ink">{{ $auditLog->target_type ?? 'N/A' }} |
            #{{ $auditLog->target_id ?? 'N/A' }}</span>
        </div>
        <div class="flex flex-col gap-1">
          <p class="text-ink-muted font-semibold text-body-mid">Created At:</p>
          <span class="text-caption text-ink">{{ $auditLog->created_at->format('d M Y, H:i') }}</span>
        </div>
      </div>
    </section>
    ...
    <section class="w-full p-6 rounded-lg border border-hairline">
      ...
      <div class="flex flex-col gap-2">
        @forelse ($auditLog->format_metadata as $label => $value)
