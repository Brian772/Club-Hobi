...
@section('content')
  ...
  <section class="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-4">
    ...
    <div class="w-full h-full flex flex-col border border-hairline rounded-lg p-4">
      ...
      <span class="text-2xl font-bold text-ink mr-8">{{ $reports->whereIn('status', ['ignored', 'resolved'])->count() }}</span>
    </div>
    <div class="w-full h-full flex flex-col border border-hairline rounded-lg p-4">
      <h3 class="text-body-mid text-ink-muted">Appeals pending</h3>
      <span class="text-2xl font-bold text-ink mr-8">7</span>
    </div>
    <div class="w-full h-full flex flex-col border border-hairline rounded-lg p-4">
      <h3 class="text-body-mid text-ink-muted">Appeals approved</h3>
      <span class="text-2xl font-bold text-ink mr-8">5</span>
    </div>
  </section>

  {{-- Reports & Appeals tables --}}
  <section x-data="{ tab: 'reports' }" class="flex flex-col mt-4 rounded-lg border border-hairline p-2 lg:p-4">
    ...
    {{-- Appeals Tab --}}
    <div x-show="tab === 'appeals'" class="mt-4">
      <h2>Appeals</h2>
    </div>
  </section>
@endsection
