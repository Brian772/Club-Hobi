...
<div class="app-layout">
    ...
    <main class="main-content">
        ...
        <form action="{{ route('settings.profile.update') }}" method="POST" class="profile-page-content">
            @csrf
            ...
            <div class="form-group-item">
                ...
                <div class="settings-group bio-group">
                    <textarea name="bio" class="custom-textarea" rows="3"
                        maxlength="150" id="bioInput">{{ old('bio', $user->bio) }}</textarea>
                </div>
            ...
            <div class="form-group-item">
                ...
                <div class="hobby-list" id="hobbyList">
                    @forelse ($user->clubs as $club)
                        <span class="hobby-badge active" data-club-id="{{ $club->id }}" onclick="openDeleteHobbyModal(this)">{{ $club->category }}</span>
                    @empty
...
<div class="profile-modal-overlay" id="avatarModal">
    <div class="profile-modal">
        ...
        <div class="modal-body">
            ...
            <form action="{{ route('settings.profile.avatar') }}" method="POST"
                enctype="multipart/form-data" class="avatar-upload-form">

                @csrf
                <input type="file" name="avatar" id="avatarInput"
                    accept=".jpg,.jpeg,.png" required onchange="showSelectedFile(this)">
            ...
            <form action="{{ route('settings.profile.hobby.add') }}" method="POST" id="hobbyForm">
                ...
                <div class="hobby-options">
                    @forelse ($clubs as $club)
                        @php
                            $alreadyJoined = $user->clubs->contains(
                                'id',
                                $club->id
                            );
                        @endphp
                        ...
                        <button type="button" class="hobby-card {{ $alreadyJoined ? 'disabled' : '' }}"
                            ...
                            <div class="hobby-card-content">
                                ...
                                <span class="hobby-sub">{{ $club->name }}</span>
                            </div>
